...
#no censoring (C = 1 implies censored)
#can use LT only results
sum(obs_data$C)

n = nrow(obs_data)

source("./code/ime-hazard-rate-functions.R")
source("./code/pl-geom-right-cens-app-formulas.R")
...
#calibrate geom dist to when loans end
est_dist_true = est_dist_true[est_dist_true$lam_hat > 0,]
est_dist_true = est_dist_true[-1,]
...
THETA_est$par = c(0.9693, 0.92485)
...
for(j in k){
  ...
  nk = append(nk, sum( obs_data$Zi == j) )
}

pdf_X = function(u, THETA){
  
  p = THETA[1]
  b = THETA[2]
  
  if( ((min(est_dist_true$Age)) <= u) & (u <= (max(est_dist_true$Age) - 1)) ){
    return( p^((u - min(est_dist_true$Age))^b) -
              p^((u - min(est_dist_true$Age) + 1)^b) )
  }
  if( u == max(est_dist_true$Age) ){
    return( p^((u - min(est_dist_true$Age))^b) )
  }
...
np0j =
  nrow(obs_data) *
  sapply(c(est_dist$Age), pdf_X, THETA = c(THETA_est$par, G_hat))

Q = sum( ((nk - np0j)^2) / np0j )
pchisq(Q, df = length(k) - 1, lower.tail = FALSE)

#LRT
l.1 = log_like_fn(c(THETA_est$par, G_hat))
pchisq(-2 * (-l.0 - (-l.1)), df = 1, lower.tail = FALSE)

haz = function(u, THETA){
  return( f_X(u, THETA) / S_X(u, THETA))
}

sapply(est_dist$Age, haz,
       THETA = c(THETA_est$par, G_hat))

sapply(est_dist$Age, haz,
       THETA = c(0.99108, 1.245, G_hat))


#2017 - 73 month loans
rm(list=ls())

obs_data = read.csv('./data-clean/aart-2017-73mo.csv')
obs_data = obs_data[,-1]

trap_param = read.csv('./data-clean/aart-2017-73mo-trapezoid-dim.csv')
