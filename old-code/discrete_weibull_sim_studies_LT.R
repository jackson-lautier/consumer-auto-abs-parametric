...
rownames(disp_mat) = c("cpu_optim", "thm_2.1", "mle")
...
disp_mat["thm_2.1", ] = c(p_hat, G_hat)
...
for(v in c((Delta + 1):(Delta + m))){
  for(u in c(v:omega)){
    cur11 = append(cur11,
                   dpsi1_dp(u,v,THETA) * h_star(u,v,THETA))
  }
}
...
for(v in c((Delta + 1):(Delta + m))){
  for(u in c(v:omega)){
    cur12 = append(cur12,
                   dpsi1_db(u,v,THETA) * h_star(u,v,THETA))
  }
}
...
for(v in c((Delta + 1):(Delta + m))){
  for(u in c(v:omega)){
    cur21 = append(cur21,
                   dpsi2_dp(u,v,THETA) * h_star(u,v,THETA))
  }
}
...
for(v in c((Delta + 1):(Delta + m))){
  for(u in c(v:omega)){
    cur22 = append(cur22,
                   dpsi2_db(u,v,THETA) * h_star(u,v,THETA))
  }
}
...
write.csv(facet_data, "fd_lt_rt_geom.csv")
